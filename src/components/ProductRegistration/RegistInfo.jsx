...
const DetailCommonWrapper = css`
  ...
  height: 35px;
  border: ${(props) =>
    props.$isSelected ? "2px solid var(--color-black)" : "1px solid var(--color-lightgray)"};
  cursor: pointer;
  &:hover {
...
const ArrowDownIcon = styled(MdKeyboardArrowDown)`
  font-size: 1.3rem;
  transition: all 0.5s;
  transform: ${(props) => (props.$isNoticeOpen ? "rotate(180deg)" : "rotate(0)")};
`;
  ...
  return (
    <Layout>
      ...
      <InputWrapper ref={categoryRef}>
        ...
        <FilterBtn type="button" onClick={() => setIsCategoryOpen((prev) => !prev)}>
        ...
        <Details key={uuid4()}>
          ...
          <Detail>
            ...
            <Quantity
              ...
              onChange={(e) => handleItemStocks(idx, "stockQty", +e.target.value)}
            />
          </Detail>
        </Details>
      ))}
      ...
      <NoticeDetail $isNoticeOpen={isNoticeOpen}>
        Every product of our online store is also on sale at DANTON's physical store,
        <br /> so it may have already been sold out when you placed your order. <br /> We hope for
...
const categories = ["MEN'S", "WOMEN'S", "BAGS", "HEADWEAR", "GOODS", "STORE EXCLUSIVE"];
